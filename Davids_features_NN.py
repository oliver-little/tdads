...
def Process(startpoint,numimages,fileflag):
    ...
    if fileflag == 0:
        images = train_images
        labels = train_labels
        save_images = "DFeatureData.npy"
        save_labels = "DFeatureLabelData.npy"
    else:
        images = test_images
        labels = test_labels
        save_images = "DTestFeatureData.npy"
        save_labels = "DTestFeatureLabelData.npy"
    ...
    for tin in range(startpoint,startpoint+numimages):
        ...
        if tin == 0:
            dimages = np.zeros((numimages,fd.size))
        dimages[tin] = fd
        dlabels[tin] = labels[tin]
        np.save(save_images,dimages)
        np.save(save_labels,dlabels)
...
#startpoint=0
#numimages=50000
#Process(startpoint,numimages,0)
#numimages=10000
#Process(startpoint,numimages,1)

train_images = np.load("DFeatureData.npy")
train_labels = np.load("DFeatureLabelData.npy")

test_images = np.load("DTestFeatureData.npy")
test_labels = np.load("DTestFeatureLabelData.npy")


model = keras.Sequential([
    keras.layers.Input(38),
...
#print(model.weights)
for x in range(10):
    print("Params important for ",x)
    for y in range(38):
        if abs(model.weights[y][x]) - 0.5 >0:
            print("    ",y,model.weights[y][x])
